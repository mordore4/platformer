...
public class Gameplay2 : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            updateTime();
            if (!pointLostTime1 && time >= 35)
            {
                score -= 1;
                pointLostTime1 = true;
            }
            if (!pointLostTime2 && time >= 45)
            {
                score -= 1;
                pointLostTime2 = true;
            }
            ...
            if (score > persistantScript.scores[2])
            {
                persistantScript.scores[2] = score;
            }

            if (time < persistantScript.timeLevel[2])
            {
                persistantScript.timeLevel[2] = time;
